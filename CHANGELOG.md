...
## [2025-12-10] - 修复星标置顶在保存/加载后丢失的问题

### 修复
- **星标置顶持久化问题**：
  - 修复星标置顶在保存云端或加载云端后丢失的问题
  - 修复星标置顶在加载快照后丢失的问题
  - 保存快照时使用本地状态（包含星标信息）而不是从数据库获取
  - 从云端加载时自动合并本地保存的星标信息
  - 添加 localStorage 持久化，确保页面刷新后星标信息不丢失
  - 在保存、编辑、删除标题/文案时自动更新星标信息的本地存储

### 技术改进
- 优化快照保存逻辑，使用本地状态确保星标信息被正确保存
- 优化数据加载逻辑，自动合并本地和云端的星标信息
- 添加星标信息的 localStorage 持久化机制
- 暴露 state 到 window 对象，支持跨页面访问

---

## [2025-12-10] - 修复数据库字段错误并优化星标显示

### 修复
- **数据库字段兼容性处理**：
  - 修复保存时数据库表缺少 `is_starred` 和 `starred_at` 字段导致的错误
  - 添加自动降级处理：如果数据库不支持星标字段，自动移除这些字段后重试保存
  - 本地状态仍保留星标信息，用于界面显示和排序功能
  - 快照加载时兼容处理，如果原数据有星标字段会尝试保留

### 优化
- **星标显示优化**：
  - 星标标记与序号结合显示，更醒目易识别
  - 桌面端和移动端序号列显示格式：⭐1、⭐2 等（星标在前，数字在后）
  - 移除了文本前的星标符号，统一在序号位置显示
  - 星标图标与数字紧密排列，间距2px，视觉更协调

- **星标排序优化**：
  - 确保星标内容永远置顶显示
  - 星标数据之间按星标时间倒序排列（最新星标的在最前）
  - 非星标数据按创建时间倒序排列
  - 排序逻辑在标题页和文案页都生效
...
## [2025-12-10] - 添加下拉列表数据条数显示和最新快照高亮功能

### 新增功能
- **下拉列表数据条数显示**：
  - 账号分类下拉列表显示每个分类的数据条数
  - 标题页显示标题数据条数，文案页显示文案数据条数
  - 格式：场景名 X条（例如：西瓜 5条）
  - 数据条数实时更新，保存、删除、加载、导入、快照加载后自动刷新

- **最新快照高亮显示**：
  - 加载云端快照时，最新一条快照高亮蓝色显示
  - 在快照名称右侧显示"最新"标签，更醒目易识别
  - 优化云端快照列表样式，提升用户体验

### 改进
- 减少用户名徽章和退出按钮间距至3px（再减2px）
- 优化云端快照列表项布局，名称和标签在同一行显示
- 确保数据条数在所有数据操作后都能及时更新

---

## [2025-12-10] - 添加保存云端按钮默认值功能

### 新增功能
- **保存云端快照默认值**：
  - 点击"保存云端"按钮时，自动填入用户名+年月日格式的默认值
  - 格式：用户名+年月日（例如：olina20251210）
  - 自动选中文本，方便用户直接输入新名称或修改
  - 在标题页和文案页都生效

### 改进
- 提升用户体验，减少手动输入时间
- 默认值包含用户名和日期，便于识别和管理快照

---

## [2025-12-10] - 优化管理页面移动端UI和布局

### 改进
- **移动端UI优化**：
  - 修复手机端下拉列表高度与其他输入框不一致问题
    - 添加更具体的选择器确保样式生效
    - 添加box-sizing和appearance属性移除浏览器默认样式
    - 确保padding与其他输入框一致
  - 减少用户名徽章和退出按钮间距至5px（总共减少3px）
  - 减少标题条数和文案条数显示框高度，优化显示效果
  - 重新排版手机端云端快照，按钮垂直排列，提升移动端体验

- **管理页面布局优化**：
  - 添加返回主页按钮到header下方，蓝色按钮白字
  - 云端快照四个按钮（保存、查看、上一页、下一页）放在一排，自动适应宽度
  - 云端快照列表项移动端：加载和删除按钮在快照右边横向排列
  - 调整返回主页按钮与数据概览section之间的间距，保持一致的视觉间距

### 技术改进
- 优化移动端媒体查询，确保按钮布局和间距正确
- 添加云端快照按钮行样式，支持自动适应宽度
- 优化云端快照列表项移动端布局，保持横向排列

---

## [2025-12-10] - 优化移动端下拉列表宽度设置

### 改进
- **移动端下拉列表宽度优化**：
  - 调整移动端账号分类下拉列表宽度为100px（固定宽度）
  - 最小宽度设置为100px，确保内容完整显示
  - 修复CSS样式优先级问题，确保移动端样式正确生效
  - 添加更具体的选择器覆盖`.field-input`的`width: 100%`设置

### 技术改进
- 在文件末尾添加移动端媒体查询，使用`!important`确保样式优先级
- 修复多个CSS规则冲突导致的宽度设置失效问题
